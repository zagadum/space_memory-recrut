...
var Locale_attributes_uk = require('./ua.attribute.json');
var Locale_attributes_pl = require('./pl.attribute.json');
var Locale_attributes_en = require('./en.attribute.json');
...
exports.default = {
    ...
    props: {
        ...
        'trans': {
            ...
            default: function _default() {
                ...
                console.log('Lang set to '+langAttribute);

                if (langAttribute=='uk'){
                    return  Locale_attributes_uk;
                }
                if (langAttribute=='pl'){
                    return Locale_attributes_pl;
                }
                if (langAttribute=='en'){
                    return Locale_attributes_en;
                }
    ...
    methods: {
        ...
        loadData: function loadData(resetCurrentPage) {
            ...
            //console.log('loadData',resetCurrentPage);
        ...
        populateCurrentStateAndData: function populateCurrentStateAndData(object) {
          ...
          //console.log('populateCurrentStateAndData');
          //console.log(object);
          if (object==undefined){
        ...
        //     });
        // },
        deleteItem: function deleteItem(url,reloadLocal=0) {
            var _this7 = this;
            _this7.reloadLocal=reloadLocal;
            //Test22
            var messageDialog=_this7.trans['deleteItemsDialog'];


            this.$modal.show({
                template: '<div class="box admin-modal"> ' +
                    '            <button type="button" class="btn-close" @click="$modal.hide(\'DeleteMeDialog\')"><img src="/images/x.svg"/></button>\n' +
                    '                <div class="box">' +
                    '<p class="box-message">'+messageDialog.text+'</p>\n' +
                    '                <div class="box-btn">\n' +
                    '                    <button type="button" class="box-btn_subprimary" @click="$modal.hide(\'DeleteMeDialog\')">'+messageDialog.no+'</button>' +
                    ' <button class="box-btn_primary" type="button" @click="deleteBtnOK2(1)">'+messageDialog.yes+'</button>\n' +
                    '                </div></div>\n' +
                    '            </div>',
             props: ['deleteBtnOK2'],

        }, {
                deleteBtnOK2: function deleteBtnOK2(mutablePublishedAt) {
                      _this7.$modal.hide('DeleteMeDialog');
                    axios.delete(url).then(function (response) {
                        if (!_this7.reloadLocal){
                            _this7.loadData();
                        }else{
                            setTimeout(function() {
                                document.location=_this7.reloadLocal;
                            }, 1000);

                        }

                        _this7.$notify({ type: 'success', title: 'Success!', text: response.data.message ? response.data.message : _this.trans[dialogType].success });
                    }, function (error) {
                        _this7.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : _this.trans[dialogType].error });
                    });
                }
        ...
        UnBlokingItem: function UnBlokingItem(url) {
            var _this7 = this;

            this.$modal.show({
                template: '<div class="box admin-modal"> ' +
                    '            <button type="button" class="btn-close" @click="$modal.hide(\'UnBlokingItem\')"><img src="/images/x.svg"/></button>\n' +
                    '                <div class="box">' +
                    '<p class="box-message">'+_this7.trans['UnBlokingItem'].text+'</p>\n' +
                    '                <div class="box-btn">\n' +
                    '                    <button type="button" class="box-btn_subprimary" @click="$modal.hide(\'UnBlokingItem\')">'+_this7.trans['UnBlokingItem'].no+'</button>' +
                    ' <button class="box-btn_primary" type="button" @click="UnblockBtnOK2(1)">'+_this7.trans['UnBlokingItem'].yes+'</button>\n' +
                    '                </div></div>\n' +
                    '            </div>',
                props: ['UnblockBtnOK2'],

            }, {
                UnblockBtnOK2: function UnblockBtnOK2(mutablePublishedAt) {
                    _this7.$modal.hide('UnBlokingItem');
                    axios.delete(url).then(function (response) {
                        _this7.loadData();
                        _this7.$notify({ type: 'success', title: 'Success!', text: response.data.message ? response.data.message : _this.trans[dialogType].success });
                    }, function (error) {
        ...
        BlokingItem: function BlokingItem(url) {
            var _this7 = this;
            this.$modal.show({
                //@submit.stop.prevent="handleSubmit"
                template:'<div class="admin-modal"> <button type="button" class="btn-close" @click="$modal.hide(\'BlokingDialog\')"><img src="/images/x.svg"/></button>\n' +
                    '        <div class="box">\n' +
                    '            <p class="box-title">'+_this7.trans['BlokingItem'].text+'</p>\n' +
                    '            <form method="post" id="formBlokingStudent"  ref="formBlokingStudent" >\n' +
                    '                <div class="form-group row">\n' +
                    '                    <div class="col-md-12">\n' +
                    '                        <label for="payment_sum">'+_this7.trans['BlokingItem'].reason+'</label>\n' +
                    '                        <div class="dis-flex">\n' +
                    '                            <textarea type="text" class="form-control"    cols="45" rows="10"   placeholder="'+_this7.trans['BlokingItem'].reason+'" id="blocking_reason" name="blocking_reason"></textarea>\n' +
                    '                        </div>\n' +
                    '                    </div>\n' +
                    '                </div>\n' +
                    '                <div class="box-btn">\n' +
                    '                    <button type="button" class="btn box-btn_subprimary" @click="$modal.hide(\'BlokingDialog\')">'+_this7.trans['BlokingItem'].no+'</button>\n' +
                    '                    <button type="button"  class="btn  box-btn_primary" @click="BlokingBtnOK(1)">'+_this7.trans['BlokingItem'].yes+'</button>\n' +
                    '                </div>\n' +
                    '            </form>' +
                    '    </div>  </div>',
                props: ['BlokingBtnOK'],
                ...
                BlokingBtnOK: function BlokingBtnOK(mutablePublishedAt) {
                    _this7.$modal.hide('BlokingDialog');
                    let bl = $('#blocking_reason').val();


                    url=url+'?blocking_reason='+bl;
                    axios.delete(url).then(function (response) {
                        _this7.loadData();
                        _this7.$notify({ type: 'success', title: 'Success!', text: response.data.message ? response.data.message : _this7.trans[dialogType].success });
                    }, function (error) {
                        _this7.$notify({ type: 'error', title: 'Error!', text: error.response.data.message ? error.response.data.message : _this7.trans[dialogType].error });
                    });
                }
        ...
        BlokingItemSm: function BlokingItemSm(url) {
            var _this7 = this;
            this.$modal.show({
                //@submit.stop.prevent="handleSubmit"
                template:'<div class="admin-modal"> <button type="button" class="btn-close" @click="$modal.hide(\'BlokingDialog\')"><img src="/images/x.svg"/></button>\n' +
        ...
        deleteItemMe: function deleteItemMe(urlMain) {

            self.url_operation=urlMain;
            var _this7 = this;
            console.log('base='+self.url_operation);
            this.$modal.show('deleteModal',{'url2':urlMain},
                {
                    delete_btn_ok: function delete_btn_ok() {
                    }
                }
            );
            ...
            //     'deleteBtnOK': event => {
            //     console.log('deleteBtnOK');
            //     console.log('url='+url);
            //     _this7.$modal.hide('deleteModal');
            //     axios.delete(url).then(function (response) {
            //         _this7.loadData();
            //         _this7.$notify({ type: 'success', title: 'Success!', text: response.data.message ? response.data.message : 'Item successfully deleted.' });
            //     }, function (error) {
            ...
            //     });
            // },
            //     'click_to_close': event => {
            //     console.log('click_to_close');
            //     _this7.$modal.hide('deleteModal');
            // }
             //   );
        ...
        deleteItemMeReload: function deleteItemMeReload() {
            var _this7 = this;
            _this7.loadData();
        },
