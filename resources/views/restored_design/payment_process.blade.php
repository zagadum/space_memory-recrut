...
<div class="pay-wrap">
    ...
    <div class="pay-grid">
        ...
        <div class="pay-card">
            <div class="pay-card__header">
                ...
                <div class="pay-card__sub">
                    Ученик: <strong style="color:#fff">{{ $student->full_name ?? 'Иванов Артём' }}</strong>
                    &nbsp;·&nbsp;
                    Группа: <strong style="color:var(--teal)">{{ $student->group->name ?? 'Группа A · Вторник 17:00' }}</strong>
                </div>
            </div>
        ...
        <div class="hist-card">
            ...
            {{-- Раскомментируй когда нет платежей: --}}
            {{--
            ...
            {{-- Пример строк (замени на @foreach из БД) --}}
            <div class="hist-row">
                <div class="hist-row__icon">
                    <i class="fas fa-check"></i>
                </div>
                <div class="hist-row__info">
                    <div class="hist-row__period">Март 2026 · 1 месяц</div>
                    <div class="hist-row__date">12.03.2026</div>
                </div>
                <div class="hist-row__amount">440 zł</div>
            </div>

            {{-- Пример ожидающего платежа --}}
            {{--
            <div class="hist-row">
                <div class="hist-row__icon hist-row__icon--pending">
                    <i class="fas fa-clock"></i>
                </div>
                <div class="hist-row__info">
                    <div class="hist-row__period">Апрель 2026 · 1 месяц</div>
                    <div class="hist-row__date">Ожидает подтверждения</div>
                </div>
                <div class="hist-row__amount hist-row__amount--pending">440 zł</div>
            </div>
            --}}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let   selected   = document.querySelector('.period-item.selected');
    ...
    // Делегирование: вешаем на родителя, а не на каждый item
    ...
    /* ── Open modal on pay click (только если не locked) ── */
    ...
    document.getElementById('confirmPay').addEventListener('click', function () {
        ...
        const months  = selected ? selected.dataset.months  : 1;
        const price   = selected ? selected.dataset.price   : 440;
        const lessons = selected ? selected.dataset.lessons : 4;

        // Отправь запрос на свой backend для создания платежа PayU
        fetch('{{ route("student.payment.create") ?? "/payments/create" }}', {
            method: 'POST',
            headers: {
