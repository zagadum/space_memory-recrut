...
@section('title', trans('admin.region.actions.index'))
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.region.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/regions/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.region.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="ceard-block">
                            <form @submit.prevent="">
                            ...
                            <table class="table table-hover table-listing">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'">{{ trans('admin.region.columns.id') }}</th>
                                        <th is='sortable' :column="'name'">{{ trans('admin.region.columns.name') }}</th>
                                        <th is='sortable' :column="'country_id'">{{ trans('admin.region.columns.country_id') }}</th>
                                        <th is='sortable' :column="'enabled'">{{ trans('admin.region.columns.enabled') }}</th>

                                        <th></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a href="#" class="text-primary" @click="onBulkItemsClickedAll('/admin/regions')" v-if="(clickedBulkItemsCount < pagination.state.total)"> <i class="fa" :class="bulkCheckingAllLoader ? 'fa-spinner' : ''"></i> {{ trans('admin.listing.check_all_items') }} @{{ pagination.state.total }}</a> <span class="text-primary">|</span> <a
                                                        href="#" class="text-primary" @click="onBulkItemsClickedAllUncheck()">{{ trans('admin.listing.uncheck_all_items') }}</a>  </span>

                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/regions/bulk-destroy')">{{ trans('admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/regions/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.region.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
