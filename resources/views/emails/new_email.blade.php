{{--<div>--}}
{{--    {{ trans('admin.admin-user.actions.change_email') }}--}}
{{--</div>--}}
{{--<div>--}}
{{--    {{ trans('admin.admin-user.actions.new_email') }}: {{ $newEmail->email }}--}}
{{--</div>--}}
{{--@if(isset($newEmail->link))--}}
{{--<form method="post"  action="{{ $newEmail->link }}">--}}
{{--    <input type="hidden" name="email" value="{{$newEmail->email}}">--}}
...
<div>
    <table style="width:90%"  align="center" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr><td style="background-color:#e5e5e5 ">
                <div id="subject"><b> {{ trans('admin.admin-user.actions.change_email') }}</b></div>
            </td></tr>
        <!-- Text -->
        <tr><td>
                <div id="content">
                    <p class="sP">Вітаю, <b>{{$newEmail->name}}</b>!</p>
                    <p class="sP">Хтось, можливо ви, ініціював процедуру зміни email в особистий кабінет  Vilno.org.</p>
                    <p class="sP">{{ trans('admin.admin-user.actions.new_email') }}: {{ $newEmail->email }}</p>

                    @if(isset($newEmail->link))
                    <p class="sP">ЯКЩО зміна пошти ініційована вами, то пройдіть по посиланню</p>
                        <form method="post"  action="{{ $newEmail->link }}">
                            <input type="hidden" name="password" value="{{$newEmail->email}}">
                            <input type="hidden" name="uuid" value="{{$newEmail->uuid}}">
