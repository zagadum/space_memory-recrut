// Создает элемент интерфейса пользователя
...
// Представляет собой бусину на абаке
...
// Контролирует поведение абака, включая инициализацию и активацию бусин

function AbacusCtrl(type) {
    this.type = type; // 0 Japanese, 1 Chinese

    this.beadLines = 8 // количество линий
    ...
    this.init = function() {
        ...
        for(var i=0; i < this.beadLines; i++) {
            for(var j=0; j < this.beadPerLine; j++) {
                ...
                bead.position[0] = 425 - i * this.beadSpacing;
                bead.position[1] = this.beadWidth + this.beadPerLine * this.beadHeight - j * this.beadHeight;
                bead.value = 1;
                if(j > this.beadSep) {
                    bead.position[1] = this.beadWidth + this.beadPerLine * this.beadHeight - (j * this.beadHeight + 2 * this.beadHeight);
                    bead.value = 5;
                }
                bead.uniqueID = id;
...
// Отвечает за создание и управление абаком
//@showNumbers отобразить внизу цифры
function Abacus(parentDivId, type,showNumbers=false) {
    var abacusCtrl = new  AbacusCtrl(type);
    var canvas;
    var divId = parentDivId;
    var beadColor = "rgba(133, 178, 255, 1.0)"; //Цвет бусин
    var hooveredBeadColor = "rgba(170, 215, 255, 1.0)";//Цвет бусин при наведении
...
// Рисует все бусины
    ...
    this.calculateAbacusResult = function () {
        var totalValue = 0;
        var placeValue = 1; // Start with the smallest place value (units)
        ...
        for(var i=0; i < abacusCtrl.beadLines; i++) {
            ...
            // Calculate the column's value
            for(var j=0; j < abacusCtrl.beadPerLine; j++) {
                var n = i * abacusCtrl.beadPerLine + j;
                if(abacusCtrl.nodes[n].active) {
                    columnValue += abacusCtrl.nodes[n].value;
                }
            }

            // Multiply by the place value and add to the total
            totalValue += columnValue * placeValue;
            placeValue *= 10; // Move to the next place value (tens, hundreds, etc.)
        }

        // Convert the total value to a string for display
        var resultString = totalValue.toString();

        // Optionally, display the result
        //console.log(resultString); // Or use another method to display the result
